...
START_TEST(test6) {
  ...
  s21_decimal res = {3456, 43234, 2134, 432};
  s21_decimal expect = {0x33333333, 0x33333333, 0x33333333, 0x0};
  set_exp(&val_1, 16);
  set_exp(&val_2, 16);
  set_exp(&expect, 15);
  // print_decimal(val_1);
...
START_TEST(test9) {
  ...
  s21_decimal res = {3456, 43234, 2134, 432};
  ;
  s21_decimal expect = {0xFFFFFFFB, 0x00000000, 0x00000004, 0x80000000};
  set_sgn(&val_1, 1);
  set_sgn(&val_2, 0);
  // print_decimal(val_1);
  // print_decimal(val_2);
  int error = s21_add(val_2, val_1, &res);
  // print_decimal(res);
  ck_assert_int_eq(error, 0);
  ck_assert_uint_eq(s21_is_equal(expect, res), 1);
}
